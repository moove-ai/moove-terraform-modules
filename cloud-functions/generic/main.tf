locals {
  build_included_files = var.build_included_files != [] ? var.build_included_files : ["cloud-functions/${var.function_name}/**"]
  build_file           = var.build_file != "" ? var.build_file : "cloud-functions/${var.function_name}/cloudbuild.yaml"
  build_tags           = [var.github_repo, "deplpy", "cloud-function"]
