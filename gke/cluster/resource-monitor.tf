...
resource "google_bigquery_data_transfer_config" "billing_query" {
  count         = var.create_resource_monitor ? 1 : 0
  depends_on = [google_project_iam_member[0].bq_transfer_permission]
  ...
  destination_dataset_id = google_bigquery_dataset.resource-monitor[0].dataset_id
  params = {
...
locals {
  ...
  FROM
    `moove-systems.billing.gcp_billing_export_v1_01C68D_9DA6E5_136FB8`
  WHERE
    usage_start_time >= TIMESTAMP("2022-12-01")
  ...
  consumption_based_cost_allocation AS (
      WITH
        ...
        SELECT
          ...
          billing_table.rate
        FROM
          `${var.project_id}.${local._cluster_name}_export.gke_cluster_resource_consumption` AS resource_usage
        INNER JOIN
          billing_table
        ON
          resource_usage.sku_id = billing_table.sku_id
