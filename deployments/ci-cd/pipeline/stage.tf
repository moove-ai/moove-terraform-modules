  ...
  build {
    ...
    step {
      ...
      args = ["-c", <<-EOF
        ...
        cd /workspace/k8s-apps
        echo $(yq '.argocdApplications.$REPO_NAME.name' apps/staging.yaml) > /workspace/name.txt
