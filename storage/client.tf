...
locals {
  client-key = <<-EOT
    ACCESS_KEY=${var.client_hmac_key ? google_storage_hmac_key.key.access_id : "null"}
    SECRET_ACCESS_KEY=${var.client_hmac_key ? google_storage_hmac_key.key.secret : "null"}
    EOT
}
